...
- **Database:** on monster, `cd ~/proj/home-db && ./add-app.sh sudokusensei`. It
  prints the URL once: put it in `deploy/.env` (mode 600), and in `.env.local` on
  the Mac as the LAN form, `postgres://sudokusensei:<pw>@192.168.50.99:5432/sudokusensei`.
  Backups come with it, from `home-db-backup.timer`.
- **Development:** `npm run dev` on the Mac against the dev database above. Tests
...
check. A handful of puzzles from a seed file.
